...
public class List 
{
	...
	private void propotionalClone()
	{
		...
		Pair[] nList = new Pair[length];	//total number of cloned antibodies to be produced
		int k=0;	//index of starting element
		for(int i=0;i<n;i++)
		{
			int c = k + (int)Math.round(list[i].Af*n/sum);	//c is index of last element 
			for(;k<c;k++)
	...
	public void antipropotionalMutation()
	{
		...
		for(int i=0;i<list.length;i++)
			list[i] = new Pair(list[i].Ab,Ag.affinity(list[i].Ab),list[i].indx);		
	}
	...
	public Result doWork()
	{
		...
		doIndxOfLowest();
		pickN();	//list now only contains n Pairs(antibody info, affinity with current antigen, index in original antibody list)
		propotionalClone();	//list now contains the clones
		antipropotionalMutation();	//list now contains pairs of mutated clones with updated afinity
